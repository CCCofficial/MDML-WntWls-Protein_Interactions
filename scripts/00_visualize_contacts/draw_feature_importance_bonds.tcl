...
proc draw_contacts {bonds colsel} {
    set myfile $bonds
    set a [open $myfile];	# 
    set content [read -nonewline $a]
    set lines [split $content "\n"]
    close $a;                          # Saves a few bytes :-)
    # Sanity check: skip if file is empty
    if {[string trim $content] eq ""} {
        puts "Warning: '$bonds' is empty. Skipping draw_contacts."
        return
    }
	...
	graphics top sphere $atom1 radius 1.5; # 
    };					       # 
	...
	draw color $colsel;			    # 
	graphics top cylinder $atom1 $atom2 radius 1; # 
    };						      # 
};						      # 

proc run {} {
    mol new ../00_map/input/Wnt8a_align.pdb
    draw_contacts ../09_model_eval/output/feature_importances_region_red.txt red
#    draw_contacts ../09_model_eval/output/feature_importances_region_green.txt green; # 
    draw_contacts ../09_model_eval/output/feature_importances_region_blue.txt blue; # 
    draw_contacts ../09_model_eval/output/feature_importances_region_purple.txt purple;	# 
    draw_contacts ../09_model_eval/output/feature_importances_region_gray.txt gray; # 
};				# 
